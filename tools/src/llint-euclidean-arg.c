...
int main(const int argc, const char * argv[]) { 
  ...
  label__body: { 
    ...
    { 
      ...
      dividend0 = llint_argv_value; 
    }; 
    
    if (2 == argc) { 
      write_long_long_int_ln(STDOUT_FILENO, dividend0); 
      goto label__exit; 
    }; 
    ...
    { 
      ...
      const int16_t prefix_bytesize = llint__token_get_number_prefix_bytesize(argv_i, &roundup_huh); 
      const long long int llint_argv_value = (roundup_huh?1:0) + convert_buffer_to_long_long_int(argv_i, prefix_bytesize); 
      divisor0 = llint_argv_value; 
    }; 
    
    long long int expected_number_of_digits_to_be_printed = 3; 
    if (4 == argc) { 
